...
public class Baek1759 {
    ...
    static void combination(int cnt, int start, int L, int C){  //cnt1,2를 처리해줘야함.
        if(cnt == L){
    ...
    static boolean isValid(char[] selected){
        ...
        for(char ch: selected){
            if(vo.contains(ch+""))  count_vo++;
            else    count_con++;
        }
        if(count_vo>=1&&count_con>=2)   return true;
        return false;
    }
}
